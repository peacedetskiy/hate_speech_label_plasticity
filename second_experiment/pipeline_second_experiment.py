...
from conditions import ACTIVE_SUITE, buildPrompt, sampleDatasetPersonas
...
_DR_COLS = ["_dr_age", "_dr_gender", "_dr_race"]
...
DEFAULT_DATASET = "datasets/measuring_hate_speech_test1000.parquet"
...
def _write_dataset(df: pd.DataFrame, filename: str):
    save_df = df.drop(columns=[c for c in _DR_COLS if c in df.columns])
...
def main(
    ...
    for filename in file_list:
        ...
        if "dataset_random" in suite and not all(c in df.columns for c in _DR_COLS):
            df = sampleDatasetPersonas(df)
